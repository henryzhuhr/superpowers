...
**Goal:** 用仅依赖 Node 内建模块的单个零依赖 `server.js`，替换 brainstorm server 中 vendored 的 node_modules。

**Architecture:** 单文件实现，包含 WebSocket 协议（RFC 6455 文本帧）、HTTP server（`http` 模块）和文件监听（`fs.watch`）。当作为模块被 require 时，导出协议函数以支持单元测试。

**Tech Stack:** 仅使用 Node.js 内建模块：`http`、`crypto`、`fs`、`path`

**Spec:** `docs/superpowers/specs/2026-03-11-zero-dep-brainstorm-server-design.md`

**Existing tests:** `tests/brainstorm-server/ws-protocol.test.js`（单元测试）、`tests/brainstorm-server/server.test.js`（集成测试）

---

## 文件映射

- **Create:** `skills/brainstorming/scripts/server.js`：零依赖替代实现
- **Modify:** `skills/brainstorming/scripts/start-server.sh:94,100`：把 `index.js` 改为 `server.js`
- **Modify:** `.gitignore:6`：移除 `!skills/brainstorming/scripts/node_modules/` 例外
...
- **Delete:** `skills/brainstorming/scripts/node_modules/`（714 个文件）
...
- Test: `tests/brainstorm-server/ws-protocol.test.js`（已存在）

- [ ] **Step 1: 在 server.js 中创建 OPCODES 常量和 computeAcceptKey**
...
服务端帧永远不带 mask。支持三种长度编码：
- payload < 126：2 字节 header（FIN+opcode、length）
- 126-65535：4 字节 header（FIN+opcode、126、16 位长度）
- &gt; 65535：10 字节 header（FIN+opcode、127、64 位长度）
...
客户端帧总是带 mask。返回 `{ opcode, payload, bytesConsumed }`；如果不完整则返回 `null`。遇到未带 mask 的帧要抛错。
...
### Task 2：添加 HTTP server、文件监听和 WebSocket 连接处理

**Files:**
- Modify: `skills/brainstorming/scripts/server.js`
- Test: `tests/brainstorm-server/server.test.js`（已存在）

- [ ] **Step 1: 在 server.js 顶部（requires 后）添加配置和常量**
...
在模块作用域加载 `frameTemplate` 和 `helperInjection`，这样 `wrapInFrame` 与 `handleRequest` 都能访问它们。它们只从 `__dirname`（scripts 目录）读文件，因此无论模块是被 require 还是直接运行都有效。
...
- [ ] **Step 5: 添加 debounce timer map**

```js
const debounceTimers = new Map();
```

文件监听逻辑会直接写在 `startServer`（Step 6）里，这样 watcher 生命周期能与 server 生命周期保持在一起，并且可以按 spec 添加 `error` handler。

- [ ] **Step 6: 添加 startServer 函数和条件式 main**

`frameTemplate` 和 `helperInjection` 已在模块作用域定义（Step 2）。`startServer` 只负责创建 screen 目录、启动 HTTP server、watcher，并记录启动信息。
...
测试目录中已经有一个带 `ws` 依赖的 `package.json`。如果需要先安装，再运行测试。
...
- Delete: `skills/brainstorming/scripts/package.json`
- Delete: `skills/brainstorming/scripts/package-lock.json`
- Delete: `skills/brainstorming/scripts/node_modules/`（整个目录）

- [ ] **Step 1: 更新 start-server.sh，把 `index.js` 改为 `server.js`**
...
- [ ] **Step 2: 移除 gitignore 中对 node_modules 的例外**
...
预期：出现等待页面，显示 "Waiting for Claude to push a screen..."

- [ ] **Step 3: 向 screen 目录写入一个 HTML 文件**

```bash
echo '<h2>Hello from smoke test</h2>' > /tmp/brainstorm-smoke/test.html
```

预期：浏览器 reload，并展示被 frame template 包裹的 "Hello from smoke test"

- [ ] **Step 4: 验证 WebSocket 正常工作，检查浏览器控制台**

打开浏览器 dev tools。WebSocket 连接应处于已连接状态（控制台无报错）。frame template 的状态指示器应显示 "Connected"。

- [ ] **Step 5: 用 Ctrl-C 停止服务端并清理**

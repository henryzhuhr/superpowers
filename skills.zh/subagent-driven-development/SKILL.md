...
**成本：**
- 子代理调用次数更多（每个任务：实现者 + 2 个审查者）
- 控制器要做更多准备工作（提前提取所有任务）
- 审查循环会增加迭代次数
- 但能早期发现问题（比事后调试更便宜）
...
- 忽略子代理的问题（回答后再让他们继续）
- 在规范符合性上接受"差不多就行"（规范审查发现的问题 = 还没完成）
- 跳过审查循环（审查发现问题 → 实现者修复 → 重新审查）
- 让实现者的自审替代真正的审查（两者都需要）
- **在规范符合性未通过之前开始代码质量审查**（顺序错误）
- 在审查仍有未解决问题时推进到下一个任务
...
**所需的 workflow skills：**
- **superpowers:using-git-worktrees** - 确保隔离的工作区（创建一个或验证已有）

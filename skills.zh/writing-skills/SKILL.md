...
| 借口 | 现实 |
|--------|---------|
| "技能显然已经很清楚" | 对你清楚 ≠ 对其他代理清楚。测试它。 |
| "这只是个参考文档" | 参考文档也可能有缺口或不清晰。测试检索。 |
| "测试是小题大做" | 未经测试的技能总有问题。15 分钟测试能省下几小时。 |
| "等有问题再测试" | 问题等于代理用不了技能。部署前先测试。 |
| "太繁琐了不想测" | 测试再繁琐，也比上线后调试有问题的技能强。 |
| "我有信心它没问题" | 过度自信必然出问题。还是要测。 |
| "学术审查就够了" | 读到 ≠ 用到。测试应用场景。 |
| "没时间测试" | 部署未经测试的技能，之后花更多时间修复更亏。 |

**这些全都意味着：先测试再部署。没有例外。**

## 为技能添加反合理化说辞

纪律约束型技能（如 TDD）需要具备抵抗合理化说辞的能力。代理很聪明，在压力下会找到漏洞。

**心理学提示：**了解说服技巧为何有效，能帮你系统化应用它们。参见 `persuasion-principles.md` 了解研究基础（Cialdini, 2021; Meincke et al., 2025），涵盖权威、承诺、稀缺、社会认同和一致性原则。
...
这能切断一整类"我在遵循精神"的合理化说辞。

### 构建合理化说辞对照表

从基线测试中捕捉合理化说辞（见下方测试章节）。代理用过的每个借口都放进表格：

```markdown
| 借口 | 现实 |
|--------|---------|
| "太简单了不用测" | 简单代码也会坏。测试只要 30 秒。 |
| "我之后会测的" | 事后测试通过只能验证"它做了什么"。 |
| "事后测试能达到同样目的" | 事后测试 = "这段代码是干什么的？" 事前测试 = "这段代码应该干什么？" |
```
...
**这些全都意味着：删除代码。用 TDD 重新开始。**
```

### 为 CSO 添加违规症状

在 description 中加入即将违反规则的征兆：
...
在没有技能的情况下用子代理运行高压场景。记录具体行为：
- 代理做了哪些选择？
- 他们用了哪些合理化说辞（原话）？
- 哪些压力触发了违规？

这就是"亲眼看到测试失败"——你必须在写技能前看到代理天然会怎么做。

### GREEN：编写最少技能

只写足以应对那些特定合理化说辞的技能。不要为假设情况添加额外内容。

在相同场景下运行，这次带上技能。代理现在应该会遵守。

### REFACTOR：封堵漏洞

代理找到了新的合理化说辞？加入明确反制。重新测试，直到固若金汤。
...
**重要：使用 TodoWrite 为下面每一项创建待办事项。**

**RED 阶段 - 编写失败测试：**
- [ ] 创建高压场景（纪律约束型技能需要 3 种以上压力组合）
- [ ] 在没有技能的情况下运行场景 - 逐字记录基线行为
- [ ] 识别合理化说辞/失败的模式

**GREEN 阶段 - 编写最少技能：**
- [ ] 名称只使用字母、数字和连字符（无括号/特殊字符）
- [ ] YAML frontmatter 包含必需的 `name` 和 `description` 字段（最多 1024 字符；见 [规范](https://agentskills.io/specification)）
- [ ] description 以 "Use when..." 开头并包含具体的触发器/症状
- [ ] description 使用第三人称
- [ ] 全文包含搜索关键词（错误信息、症状、工具）
- [ ] 清晰的概览，包含核心原则
- [ ] 针对 RED 阶段识别的具体基线失败
- [ ] 代码内联或链接到独立文件
- [ ] 一个优秀的示例（不是多语言）
...
**针对这个流程做优化** - 尽早并频繁地放置可搜索的关键词。

## 核心结论

**创建技能就是把 TDD 应用到流程文档上。**

同样的铁律：没有失败的测试，就没有技能。
同样的循环：RED （基线）→ GREEN （写技能）→ REFACTOR （封堵漏洞）。
同样的好处：更高质量、更少意外、更可靠的结果。

如果你为代码遵循 TDD，那就为技能也遵循 TDD。这是同样的纪律，只不过应用到了文档上。
